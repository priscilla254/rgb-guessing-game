...
const colorDispayElement=document.getElementById("color-display")
...
startButton.addEventListener("click", function(){
    ...
    for (let i=0;i<squares.length;i++){
        ...
        // storing values in a HTML element
...
function setHeaderRgbBackgroundColor(squareElement){
    ...
    colorDispayElement.dataset.rgb_value=rgbString;
...
// add eventlistener to squares so that it disappears or changes to uniform color

squares.forEach(square =>{
    square.addEventListener("click",function(){
        const headerRgbValue=colorDispayElement.dataset.rgb_value
...
function setSquareBackgroundAfterWin(headerRgbString){
    ...
    squares.forEach(sq=>{
        ...
        sq.dataset.rgb_value=colorDispayElement.dataset.rgb_value
